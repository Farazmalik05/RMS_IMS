                            ...
                            <table id="quotes-list" class="table text-nowrap mb-0 fs-14 table-styling">
                                ...
                                <tbody class="list contact-list-container">
                                    @foreach($invoices as $key=>$invoice)
                                        ...
                                        @php $jobs = implode("\n", $jobs); @endphp
                                        <tr title="{{ $jobs }}">
                                            <td scope="row" class="td">{{ date('d-m-Y', strtotime($invoice->quote_date)) }}</td>
                                            <td class="quote_no td">INV/RMS/{{ date('Y', strtotime($invoice->quote_date)) }}/{{ sprintf("%02d", $invoice->quote_number) }}</td>
                                            <td class="name td">{{ $invoice->vehicle->customer->name }}</td>
                                            <td class="name td">{{ $invoice->vehicle->customer->phoneno }}</td>
                                            <td class="phone td">{{ $invoice->vehicle->rego }}</td>
                                            <td class="rego td">{{ $invoice->vehicle->make." ".$invoice->vehicle->model." - ".$invoice->vehicle->year }}</td>
                                            <td class="amtdue td">
                                                @if($invoice->amtdue <= 0)
                                            ...
                                            <td class="total td">${{ number_format((float)$invoice->total, 2, '.', '') }}</td>
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    <a href="{{ route('invoices.show', $invoice) }}" class="btn btn-ghost btn-icon btn-sm rounded-circle" title="View">
                                                    ...
                                                    <button id="mark-paid" data-id="{{ $invoice->id }}" class="btn btn-ghost btn-icon btn-sm rounded-circle" title="Mark as unpaid">
                                                    ...
                                                    <a href="javascript:{}" onclick="event.preventDefault();document.getElementById('delete-invoice-{{ $invoice->id }}').submit();" class="btn btn-ghost btn-icon btn-sm rounded-circle" title="Delete">
                                                    ...
                                                    <form id="delete-invoice-{{ $invoice->id }}" action="{{ route('invoices.destroy', $invoice) }}" method="post">
                                                        @csrf
                                                        @method('delete')
                                                    </form>
                                                </div>
