            ...
            <div class="card">
                <div class="card-header d-lg-flex justify-content-between ">
                    <div class="d-grid d-lg-block">
                        <a href="{{ route('jobs.create') }}" class="btn btn-primary" id="new-job">
                            {{ __('+ Create New Job') }}
                        </a >
                    </div>
                </div>
                                ...
                                <tbody class="list contact-list-container">
                                    @foreach($jobs as $key=>$job)
                                        <tr>
                                            ...
                                            <td class="name td">
                                                @if (strlen($job->name) > 100)
                                                    {{ substr($job->name, 0, 100)."..." }}
                                                @else
                                                    {{ $job->name }}
                                                @endif
                                            </td>
                                            ...
                                            <td class="phone td">{{ $job->quantity }}</td>
                                            <td class="address td">{{ $job->rate }}</td>
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    <a href="{{ route('jobs.edit', $job) }}" class="btn btn-ghost btn-icon btn-sm rounded-circle">
                                                    ...
                                                    <a href="javascript:{}" onclick="event.preventDefault();document.getElementById('delete-job-{{ $job->id }}').submit();" class="btn btn-ghost btn-icon btn-sm rounded-circle">
                                                    ...
                                                    <form id="delete-job-{{ $job->id }}" action="{{ route('jobs.destroy', $job) }}" method="post">
                                                        @csrf
                                                        @method('delete')
                                                    </form>
                                                </div>
